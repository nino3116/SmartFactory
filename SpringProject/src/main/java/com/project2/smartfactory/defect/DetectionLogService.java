...
public class DetectionLogService {
    ...
    public Map<String, Object> getChartData(int totalTasks) {
        ...
        if (logs.isEmpty()) {
            ...
            // 빈 데이터로 초기화된 Map 반환
             return Map.of(
                "overallStatus", Map.of("labels", List.of("Normal", "Defect Detected"), "data", List.of(0L, 0L)),
                "weeklyDefectTrend", Map.of("labels", List.of(), "data", List.of()),
                "yearlyDefectTrend", Map.of("labels", List.of(), "data", List.of()),
                "dailyStatus", Map.of("labels", List.of("Normal", "Defect Detected"), "data", List.of(0L, 0L)),
                // Map.of 수정: key-value 쌍이 맞도록 수정
                "dailyTaskCompletion", Map.of(
                    "labels", List.of("오늘 작업"),
                    "datasets", List.of(
                        Map.of("label", "완료", "data", List.of(0L)), // Map.of 수정
                        Map.of("label", "미완료", "data", List.of((long) totalTasks)) // Map.of 수정
                    )
                )
             );
        }
        ...
        chartData.put("overallStatus", Map.of(
                ...
                "data", List.of(normalCount, defectCount)
        ));
        logger.debug("Overall status: Normal={}, Defect Detected={}", normalCount, defectCount); // logger 사용
        ...
        // 3. 연간 불량 감지 추이 (선형 차트 - 최근 12개월 불량 개수)
        logger.debug("Calculating yearly defect trend."); // logger 사용
        Map<String, Long> monthlyDefectCounts = new TreeMap<>(); // 월 순서 유지를 위해 TreeMap 사용
        ...
        for (int i = 11; i >= 0; i--) {
            ...
            monthlyDefectCounts.put(date.format(monthFormatter), 0L); // 불량 개수 0으로 초기화
        }
        ...
        logs.stream()
                .filter(log -> {
                    ...
                    LocalDate logDate = log.getDetectionTime().toLocalDate();
                    // --- 수정된 부분 끝 ---
                    // 최근 12개월 이내의 로그 중 "Defect Detected" 상태인 로그만 필터링
                    return !logDate.isBefore(twelveMonthsAgo) && "Defect Detected".equals(log.getStatus());
                })
                .forEach(log -> {
                    // --- 수정된 부분 시작 ---
                    // log.getDetectionTime()이 LocalDateTime을 반환하므로 toLocalDate() 직접 호출
                    ...
                    String monthKey = logDate.format(monthFormatter);
                     // 해당 월의 불량 개수를 합산 (defectCount가 null이면 0으로 처리)
        ...
        chartData.put("yearlyDefectTrend", Map.of(
                "labels", monthlyDefectCounts.keySet(),
                "data", monthlyDefectCounts.values()
        ));
        logger.debug("Yearly defect trend: {}", monthlyDefectCounts); // logger 사용
        ...
        chartData.put("dailyTaskCompletion", Map.of(
                ...
                "datasets", List.of(
                        Map.of("label", "완료", "data", List.of(completedTasks)), // Map.of 수정
                        Map.of("label", "미완료", "data", List.of(incompleteTasks)) // Map.of 수정
                )
        ));

	...
	try {
		...
		defectTableBody.querySelectorAll("tr").forEach((row) => {
			...
			if (cells.length > 1) {
				...
				const key = `${cells[0].textContent}-${cells[1].textContent}-${cells[2].textContent}-${cells[3].textContent}-${cells[4].textContent}-${cells[5].querySelector("img")?.src || "no-image"}`;
...
async function fetchAndDisplayDetectionLogs() {
	const logTableBody = document.querySelector("#detection-log-table tbody");
	if (!logTableBody) {
		console.error(
	...
	// 로딩 메시지 표시 (첫 로딩 시 또는 데이터가 비어있을 때)
	if (
		logTableBody.rows.length === 0 ||
		logTableBody.rows[0].cells.length === 1
	) {
	...
	try {
		...
		// 기존 행들을 Map에 저장하여 비교 및 업데이트 효율화
		const existingLogRows = new Map();
		logTableBody.querySelectorAll("tr").forEach((row) => {
			// 각 로그 행을 고유하게 식별할 수 있는 키 생성 (detectionTime 사용)
			// detectionTime은 ISO 8601 문자열 형태라고 가정
			const cells = row.cells;
			if (cells.length > 1) {
				// 로딩/데이터 없음 메시지 행이 아닌 경우
				const key = cells[0].textContent; // detectionTime
				existingLogRows.set(key, row);
			}
		});

		const newLogTbody = document.createElement("tbody");
		let logDataChanged = false; // 로그 데이터 변경 여부 플래그

		if (logs && logs.length > 0) {
			// 새로 가져온 로그 데이터를 순회하며 테이블 업데이트
			// API가 최신 로그부터 반환한다고 가정하고 순회
			logs.forEach((log) => {
				const logTimeStr = log.detectionTime
					? new Date(log.detectionTime).toLocaleString()
					: "-";
				const newLogKey = logTimeStr; // 포맷된 시간 문자열을 키로 사용

				if (existingLogRows.has(newLogKey)) {
					// 이미 존재하는 로그이면 기존 행을 그대로 사용 (순서 유지를 위해 newLogTbody에 추가)
					newLogTbody.appendChild(existingLogRows.get(newLogKey));
					existingLogRows.delete(newLogKey); // 사용한 행은 Map에서 제거
				} else {
					// 새로운 로그이면 새 행 생성 및 newLogTbody에 추가
					const row = document.createElement("tr");
					row.classList.add("hover:bg-gray-50", "new-row"); // 새 행에 애니메이션 클래스 추가
					logDataChanged = true; // 데이터 변경 감지

					let timeCell = document.createElement("td");
					timeCell.classList.add("py-2", "px-4", "border-b");
					timeCell.textContent = logTimeStr;
					...
					summaryCell.textContent = log.defectSummary || "-";
					...
					newLogTbody.appendChild(row); // 새 행 추가
				}
			});

			// 기존에 있었지만 새로 가져온 데이터에 없는 행들은 제거
			existingLogRows.forEach((row) => {
				row.remove(); // DOM에서 행 제거
				logDataChanged = true; // 데이터 변경 감지
			});

			// 새로운 tbody로 교체 (DOM 조작 최소화)
			// 데이터가 변경되었거나 행 수가 다르면 교체
			if (logDataChanged || logTableBody.rows.length !== logs.length) {
				logTableBody.parentNode.replaceChild(newLogTbody, logTableBody);
				newLogTbody.id = "detection-log-table-body"; // ID 유지 또는 새로운 ID 할당
			}
		} else {
			logDataChanged = true; // 데이터가 비어있으면 변경된 것으로 간주
			logTableBody.innerHTML = `
                  <tr>
                    <td colspan="5" class="py-4 px-4 text-center text-gray-500">
                        감지 로그가 없습니다.
                ...
                `;
			// 새로운 tbody로 교체 (데이터 없음 메시지 포함)
			const emptyLogTbody = document.createElement("tbody");
			emptyLogTbody.id = "detection-log-table-body";
			emptyLogTbody.innerHTML = `
           <tr>
             <td colspan="5" class="py-4 px-4 text-center text-gray-500">
               감지 로그가 없습니다.
             </td>
       ...
       `;
			logTableBody.parentNode.replaceChild(emptyLogTbody, logTableBody);
		}
		...
		// 오류 발생 시 메시지 표시 및 기존 행 모두 제거
		if (
			logTableBody.rows.length > 1 ||
			(logTableBody.rows.length === 1 && logTableBody.rows[0].cells.length > 1)
		) {
			// 기존에 데이터가 있었다면 변경된 것으로 간주
		}
		logTableBody.innerHTML = `
            ...
            `;
		// 새로운 tbody로 교체 (오류 메시지 포함)
		const errorLogTbody = document.createElement("tbody");
		errorLogTbody.id = "detection-log-table-body";
     ...
     `;
		logTableBody.parentNode.replaceChild(errorLogTbody, logTableBody);
...
function updateCharts(logs) {
	...
	if (hasData) {
		const monthlyDefectCounts = logs.reduce((acc, log) => {
			...
			if (logDate >= twelveMonthsAgo) {
				...
				if (log.status === "Defect Detected" && log.defectCount > 0) {
					acc[yearMonthString] = (acc[yearMonthString] || 0) + log.defectCount;
				} else if (
					log.status === "Normal" &&
					(acc[yearMonthString] === undefined || acc[yearMonthString] === null)
				) {
	...
	const ctx1 = document.getElementById("detectionStatusChart").getContext("2d");
